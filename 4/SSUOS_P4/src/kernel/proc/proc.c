...
pid_t proc_create(proc_func func, struct proc_option *opt, void* aux)
{
	...
	*(--top) = 6; //edi
	...
	/* You should modify this function... */
	proc_getpos(p->priority, &idx);			// get idx
...
void proc_sleep(unsigned ticks)
{
	...
	// process wake time = current ticks + running ticks
...
void kernel1_proc(void* aux)
{
	...
	while(1)
	{
		...
		if (!passed && cur_process->time_used >= 140) {
			proc_sleep(60);
			passed++;
		}
		else if (cur_process->time_used >= 200) {
			proc_end();
			break;
		}
	}
...
void kernel2_proc(void* aux)
{
	...
	while(1)
	{
		...
		if (!passed && cur_process->time_used >= 100) {
			proc_sleep(60);
			passed++;
		}
		else if (cur_process->time_used >= 120) {
			proc_end();
			break;
		}
	}
...
void kernel3_proc(void* aux)
{
	...
	while(1)
	{
		...
		if (!passed1 && cur_process->time_used >= 50) {
			proc_sleep(60);
			passed1++;
		}
		else if (!passed2 && cur_process->time_used >=100) {
			proc_sleep(60);
			passed2++;
		}
		else if (cur_process->time_used >= 150) {
			proc_end();
			break;
		}
	}
